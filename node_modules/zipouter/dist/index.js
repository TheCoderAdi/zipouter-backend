...
const extractAndProcessZip = async (zipFileName) => {
    let actualFilePath = "";
    if (!zipFileName)
        throw new ZipNotFoundError("No zip file found.");
    const unzipFolderPath = path.join(process.cwd(), zipFileName.replace(".zip", ""));
    const extractZip = (zipFileName, unzipFolderPath) => {
        const zip = new AdmZip(zipFileName);
        zip.extractAllTo(unzipFolderPath, true);
    };
    const compileAndRunJavaFiles = async (actualFilePath, unzipFolderPath) => {
        const files = fs
            .readdirSync(path.join(unzipFolderPath, actualFilePath))
            .filter((file) => file.endsWith(".java") || file.endsWith(".js") || file.endsWith(".py"));
        ...
        for (const file of files) {
            ...
            if (file.endsWith(".js")) {
                ...
                try {
                    const { stdout: runOutput, stderr: runError } = await execAsync(runCommand);
                    if (runError) {
                ...
                const runCommand = `java -classpath ${path.join(unzipFolderPath, actualFilePath)} ${className}`;
                try {
    ...
    const compilationResults = await compileAndRunJavaFiles(actualFilePath, unzipFolderPath);
